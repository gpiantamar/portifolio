  ...
  function initProjectsCarousel() {
    ...
    const setupPagination = () => {
      ...
      updateDots = () => {
        ...
        const step = getStepWidth();
        const pageIndex = Math.min(dots.length - 1, Math.round(grid.scrollLeft / step));
    ...
    const advanceSlide = () => {
      ...
      if (currentScroll >= maxScroll - 15 || maxScroll <= 0) {
        ...
        grid.scrollTo({ left: Math.min(maxScroll, currentScroll + step), behavior: "smooth" });
      }
    };
    ...
    if (prevBtn) {
      prevBtn.addEventListener("click", () => {
        ...
        if (currentScroll <= 15) {
          ...
          grid.scrollTo({ left: Math.max(0, currentScroll - step), behavior: "smooth" });
        }
        resetAutoplay();
      });
    }
